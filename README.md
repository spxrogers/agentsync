...
| --- | --- | --- |
| **Claude Code** | ✓ adapter | MCP, memory, skills, subagents, commands, hooks (◐, command hooks; other handler types reported). LSP skipped unless provided by Claude plugins. |
...
| `AGENTSYNC_TARGET_ROOT` | Redirect `$HOME` for testing (used by the hermetic test container). A sandbox: every path agentsync resolves — the canonical source included — lands under it, and `AGENTSYNC_HOME` / `GROK_HOME` are ignored while it is set. |
| `GROK_HOME` | Grok Build's own home override, honoured at user scope (absolute path; see [Grok support](docs/grok.md)). `/` and your home directory are refused. |
