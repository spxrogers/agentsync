...
description: 'Multi-round adversarial code review loop — four specialized agents (correctness,
  ...
  exhausted — whichever comes first, with two ask-first soft stops in between
  (a NIT-only round; severity declining two rounds straight) so the loop pauses
  to check in on diminishing returns instead of always running to the cap. Use
  when the user wants a thorough review of a substantive PR — new interfaces,
