...
func pluginUpgradeRun(cmd *cobra.Command, args []string, lossless bool) error {
	...
	// defaultMarketplaceSentinel (bare-id install) names no real marketplace —
	...
	if mpName == defaultMarketplaceSentinel {
		mpName = ""
	}
...
func checkPluginRefMarketplace(existing source.Plugin, id, typedMP string) error {
	...
	// A bare-id install records defaultMarketplaceSentinel
	...
	// guard should make).
	if recorded == "" || recorded == defaultMarketplaceSentinel {
...
const defaultMarketplaceSentinel = "default"

// resolveMarketplaceName returns the marketplace name, folding an empty one
// to defaultMarketplaceSentinel.
func resolveMarketplaceName(name string) string {
	if name == "" {
		return defaultMarketplaceSentinel
	}
	return name
}

// recordedMarketplace returns the marketplace a stored plugin id
// ("<id>@<marketplace>") names, folding an unqualified id to
// defaultMarketplaceSentinel. The sentinel names no marketplace: the poll
// engine's fetched index is keyed by REGISTERED marketplace names
// (plugin_poll.go), so it has no entry for the sentinel unless a marketplace
// is literally registered as "default" (nothing reserves the name — see the
// constant's doc), and a bare-id install is therefore invisible to `plugin
// outdated` and `plugin upgrade --all` (#251). This helper only unifies the
// spelling of splitPluginRef + the sentinel default; it does not make that
// lookup work.
