...
func TestApplyPipelineLoadsStateAfterSourceReload(t *testing.T) {
	...
	t.Run("pipeline loads state after the source reload", func(t *testing.T) {
		...
		// shrink the text under test. A complete capture ends with the
		// function's own final statement — its top-level `return nil`, indented
		// one tab — so anything else at the end means the capture was cut short
		// somewhere, possibly below the calls this guard asserts on.
		if !strings.HasSuffix(body, "\n\treturn nil") {
			t.Fatal("funcBody captured a truncated runApplyPipeline (the body does not end at the " +
				"function's top-level `return nil`). Either a column-0 '}' inside the function cut the " +
				"capture short (fix funcBody or the function), or the function's final statement changed — " +
				"then update this check")
		}
