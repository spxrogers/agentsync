...
func (v Vault) WriteVerified(plain []byte) error {
	...
	// os.IsNotExist arm in internal/cli/secrets.go), so nothing else refuses a
	// non-regular vault first. A non-regular path simply reports no previous
	// vault to preserve, which is the truth: a FIFO is not a vault.
