...
// ~/.codeium/windsurf/mcp_config.json carrying an unmodeled integer beyond
// float64's 2^53 (2^53+1). The UseNumber decode (jsonkeys.DecodeObject) must
// keep the exact digits in the passthrough Extra map — a plain json.Unmarshal
// rounds it to 9007199254740992 — and the render-back must land the literal
// digits on disk.
func TestIngest_PreservesLargeIntegerExtra(t *testing.T) {
	...
	if !bytes.Contains(data, []byte("9007199254740993")) || bytes.Contains(data, []byte("9007199254740992")) {
		t.Fatalf("re-rendered mcp_config.json lost integer precision: %s", data)
	}
}
