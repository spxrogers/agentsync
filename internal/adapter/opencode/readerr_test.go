...
func TestIngest_UnreadableMemory_FailsLoud(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tmp := t.TempDir()
			memPath := filepath.Join(tmp, ".config", "opencode", "AGENTS.md")
			tt.setup(t, memPath)
			a := opencode.New(opencode.Options{TargetRoot: tmp, Stderr: io.Discard})
...
// TestIngest_UnreadableSettings_FailsLoud: a corrupt-but-present opencode.json now
// fails loudly (hujson/JSON parse error returned) rather than reading as "no MCP".
func TestIngest_UnreadableSettings_FailsLoud(t *testing.T) {
	tests := []struct {
		...
		{name: "valid settings without mcp key is a clean no-op", setup: func(t *testing.T, settingsPath string) { plantContent(t, settingsPath, `{"theme":"dark"}`) }},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err == nil {
					t.Fatalf("expected a loud ingest error for a corrupt/unreadable opencode.json, got nil (mcp=%+v)", got.MCPServers)
				}
				return
			}
			if err != nil {
