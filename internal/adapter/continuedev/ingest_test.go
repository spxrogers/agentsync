...
func TestRoundTrip_Command(t *testing.T) {
	...
	// `name` is the canonical identity (Command.Name), regenerated by Render — it
	// must never be captured into the canonical command frontmatter.
	if _, ok := got.Commands[0].Frontmatter["name"]; ok {
		t.Fatalf("name should not be captured into canonical frontmatter: %+v", got.Commands[0].Frontmatter)
	}
}

// TestIngest_Command_NameFromFrontmatter is the artifact-anchored guard for the
// silent-rename bug: a foreign prompt block whose filename differs from its
// frontmatter `name` must be captured under the frontmatter `name` (Continue's
// slash-command identity), and re-render must produce the block under that name
// ON DISK — proving the identity survives the round trip, not just the model.
func TestIngest_Command_NameFromFrontmatter(t *testing.T) {
	tmp := t.TempDir()
	promptsDir := filepath.Join(tmp, ".continue", "prompts")
	if err := os.MkdirAll(promptsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	// Filename `foo` deliberately differs from frontmatter name `bar`; Continue
	// invokes this as /bar, so the canonical identity must be `bar`.
	native := "---\nname: bar\ndescription: a foreign command\ninvokable: true\n---\nDo the thing.\n"
	if err := os.WriteFile(filepath.Join(promptsDir, "foo.md"), []byte(native), 0o644); err != nil {
		t.Fatal(err)
	}
	a := continuedev.New(continuedev.Options{TargetRoot: tmp})
	got, err := a.Ingest(adapter.ScopeUser, "")
	...
	if len(got.Commands) != 1 {
		t.Fatalf("expected exactly one command, got %+v", got.Commands)
	}
	cmd := got.Commands[0]
	if cmd.Name != "bar" {
		t.Fatalf("command name should come from frontmatter `name` (bar), not the filename, got %q", cmd.Name)
	}
	if cmd.Body != "Do the thing.\n" {
		t.Fatalf("command body lost: %q", cmd.Body)
	}
	if cmd.Frontmatter["description"] != "a foreign command" {
		t.Fatalf("description lost: %+v", cmd.Frontmatter)
	}
	if _, ok := cmd.Frontmatter["name"]; ok {
		t.Fatalf("name must not be captured into canonical frontmatter (Render regenerates it): %+v", cmd.Frontmatter)
	}

	// Artifact-anchored fidelity: re-render the captured canonical and assert the
	// authoritative ON-DISK block is now bar.md carrying `name: bar` — Continue
	// would still invoke it as /bar. Render must not target the old foo.md path.
	ops, _, err := a.Render(secrets.ForRender(got), adapter.ScopeUser, "")
	if err != nil {
		t.Fatalf("render: %v", err)
	}
	for _, op := range ops {
		if op.Path == filepath.Join(promptsDir, "foo.md") {
			t.Fatalf("re-render must not write foo.md; the canonical identity is bar: %+v", op)
		}
	}
	if err := a.Apply(ops, adapter.PassThroughWriter{}); err != nil {
		t.Fatalf("apply: %v", err)
	}
	data, err := os.ReadFile(filepath.Join(promptsDir, "bar.md"))
	if err != nil {
		t.Fatalf("re-rendered block .continue/prompts/bar.md missing: %v", err)
	}
	for _, want := range []string{"name: bar", "invokable: true"} {
		if !strings.Contains(string(data), want) {
			t.Fatalf("re-rendered bar.md missing %q:\n%s", want, data)
		}
	}
}

// TestIngest_Command_FilenameFallback verifies the filename fallback is preserved
// when a foreign block's frontmatter carries no `name` key.
func TestIngest_Command_FilenameFallback(t *testing.T) {
	tmp := t.TempDir()
	promptsDir := filepath.Join(tmp, ".continue", "prompts")
	if err := os.MkdirAll(promptsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	// No `name` key → identity falls back to the filename `deploy`.
	native := "---\ndescription: from the filename\ninvokable: true\n---\nbody\n"
	if err := os.WriteFile(filepath.Join(promptsDir, "deploy.md"), []byte(native), 0o644); err != nil {
		t.Fatal(err)
	}
	a := continuedev.New(continuedev.Options{TargetRoot: tmp})
	got, err := a.Ingest(adapter.ScopeUser, "")
	...
	if len(got.Commands) != 1 || got.Commands[0].Name != "deploy" {
		t.Fatalf("expected filename-fallback name `deploy`, got %+v", got.Commands)
	}
}
