...
// TestSetStderr_NilResetsToDefault mirrors the other adapters' nil-reset test.
// Continue emits the lenient-YAML warning while parsing a prompt block's
// frontmatter (.continue/prompts/*.md).
func TestSetStderr_NilResetsToDefault(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(promptsDir, "bad.md"),
...
// TestSetStderr_SnapshotAtIngestEntry mirrors the other adapters' snapshot test.
func TestSetStderr_SnapshotAtIngestEntry(t *testing.T) {
	...
	for _, name := range []string{"first", "second"} {
		body := "---\nname: " + name + "\ndescription: Triggers on: rebase\n---\nbody\n"
		if err := os.WriteFile(filepath.Join(promptsDir, name+".md"), []byte(body), 0o644); err != nil {
			t.Fatal(err)
		}
	}
	...
	a := continuedev.New(continuedev.Options{TargetRoot: tmp, Stderr: primary})
