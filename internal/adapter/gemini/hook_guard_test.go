...
func TestRefusedHookEvents_StructuralVsSemantic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			`{ "BeforeTool": [ { "matcher": "Bash", "hooks": [ { "type": "command", "command": "x", "failClosed": true } ] } ] }`, true,
		},
		{
		...
		},
		{
			...
			`{ "BeforeTool": [ { "matcher": "Bash", "hooks": [ { "type": "command", "failClosed": true } ] } ] }`, true,
		},
		{
			"semantic: unmodeled handler field with a non-string command (unmodeled wins over the non-string-command structural check)",
			`{ "BeforeTool": [ { "matcher": "Bash", "hooks": [ { "type": "command", "command": 123, "failClosed": true } ] } ] }`, true,
		},
		{
