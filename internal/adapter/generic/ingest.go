...
func (a *Adapter) Ingest(scope adapter.Scope, project string) (source.Canonical, error) {
	...
	if mcpPath := a.mcpPath(scope, project); mcpPath != "" {
		if data, err := os.ReadFile(mcpPath); err == nil {
			// JSONC-tolerant: standardize comments/trailing commas before parsing
			// so a hand-edited settings file (Zed, Copilot, Amp) ingests cleanly.
			std := data
			if v, perr := hujson.Parse(data); perr == nil {
				v.Standardize()
				std = v.Pack()
			}
