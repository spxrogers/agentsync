...
// Roo emits the lenient-YAML warning while parsing a command's frontmatter
// (.roo/commands/*.md).
func TestSetStderr_NilResetsToDefault(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(cmdDir, "bad.md"),
...
func TestSetStderr_SnapshotAtIngestEntry(t *testing.T) {
	...
	a := roo.New(roo.Options{TargetRoot: tmp, Stderr: primary})
