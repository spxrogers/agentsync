...
the rest are escape hatches you'll rarely need. This page covers every variable
the CLI itself reads; the test harness's own `AGENTSYNC_TEST_*` / `AGENTSYNC_LIVE_*`
signals (beyond `AGENTSYNC_TEST_IN_CONTAINER`) are contributor-only and live in
`scripts/test-in-container.sh` and the `justfile`.
