...
the rest are escape hatches you'll rarely need. The full table lists every
variable the CLI itself reads, `AGENTSYNC_*` or not — a guard test in the
repository fails when the code reads one this page does not list. The test
