...
and `agentsync doctor`, while an absent or empty `backend` means no secrets
backend is configured and both commands skip the block; `apply` treats an
unrecognised *or* empty backend as *no* backend, so every `${secret:…}` fails to
resolve.
