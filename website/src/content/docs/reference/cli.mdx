...
otherwise silently ignore) rather than accept-and-ignore them. Every run that
renders the dashboard ends with a one-line hint pointing at `--legend`; the
"no agents enabled" short-circuit prints nothing, since there's no report to
explain.
...
| `--legend` | status | Print a standalone glossary of all nine drift classification statuses (including `clean`/`converged` spelled out separately) and exit, skipping the drift scan. Cannot be combined with `--json`, `--exit-code`, or `--agents` — each would otherwise be silently ignored, so it errors instead. A `status` run that renders the dashboard hints at this flag. |
