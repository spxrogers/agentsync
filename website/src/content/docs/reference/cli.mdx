...
first apply overwrites a dir, agentsync records a **pre-apply baseline** commit of
its prior contents, so the apply checkpoint's parent is the genuine pre-apply state
and `revert` right after a first apply restores it. The first apply to an untracked
