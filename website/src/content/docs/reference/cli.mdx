...
away — and exits without running the drift scan at all. A normal `status` run
always ends with a one-line hint pointing at `--legend`.
...
| `--legend` | status | Print a standalone glossary of all nine drift classification statuses (including `clean`/`converged` spelled out separately) and exit, skipping the drift scan. A normal `status` run always hints at this flag. |
