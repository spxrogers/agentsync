...
|---|---|---|
| `--scope user\|project` | init, verify, import, apply, status, diff, reconcile, migrate, plugin upgrade | Force the config scope. |
| `--project <path>` | init, verify, import, apply, status, diff, reconcile, migrate, plugin upgrade | Target / apply a specific project's tree. |
...
| `--json` | status, diff, plugin explain | Emit the structured payload to stdout (advisory diagnostics still go to stderr). `plugin explain --json` emits the translation `rows`. |
