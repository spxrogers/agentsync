...
agentsync auto-detects scope by looking for a `.agentsync/` tree, but you can be
explicit:

```bash
agentsync apply --scope user             # ignore any project tree
...
<Aside type="note">
	...
	`reconcile`, and `update` — anywhere the active config set matters.
</Aside>
