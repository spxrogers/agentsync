...
`mcp/`, `lsp/`, `skills/`, `subagents/`, `commands/`, `hooks/`, `memory/` (with
...
`plugin upgrade --all`, `check`) refuses to run against a project that declares none.
...
<Aside type="note">
	...
	`reconcile`, `check`, `migrate`, `mcp`, and every `agent` subcommand — anywhere the
