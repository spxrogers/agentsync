...
For non-interactive runs (cron, CI, a `plugin upgrade --all`), drive reconcile with
...
| `--auto-safe` | Resolve nothing; list every item as left unresolved. |

```bash
agentsync reconcile --auto-safe
```

`--auto-safe` is the conservative choice for automation: nothing that reaches
`reconcile` is safe to resolve unattended (`converged` and `pending` never reach
it — the next `apply` handles those), so it changes nothing and reports what
needs a human.
