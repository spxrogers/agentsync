...
### `agent add cursor` is rejected
The Cursor adapter is a no-op in v1 — still planned. (`agent add codex` works.)
Register cursor anyway with `AGENTSYNC_ALLOW_UNIMPLEMENTED=1` — apply will emit
zero ops for it.
